...
public class TraitNaturalFisher extends Trait implements EventInterface {
	...
	Boolean flag = PlayerDataHandler.get(player).getSkillInfo(this.getParentSkill()).isUnlocked(this);
	...
	if (!event.isCanceled() && player.isSneaking() && player.getHeldItemMainhand().isEmpty() && event.getHand() == EnumHand.MAIN_HAND && player.isSwingInProgress == false && flag == true
			|| GenericID != 0 && !event.isCanceled() && player.getHeldItemMainhand().isEmpty() && event.getHand() == EnumHand.MAIN_HAND && flag == true) {
