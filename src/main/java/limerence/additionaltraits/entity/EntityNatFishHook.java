...
public class EntityNatFishHook extends Entity implements IEntityAdditionalSpawnData{

	private static final DataParameter<Integer> DATA_HOOKED_ENTITY = EntityDataManager.createKey(EntityNatFishHook.class, DataSerializers.VARINT);
	private static final DataParameter<Integer> POSITION_SYNC = EntityDataManager.createKey(EntityNatFishHook.class, DataSerializers.VARINT);
		...
		protected void entityInit() {
			...
			this.getDataManager().register(POSITION_SYNC, 0);
		...
		public void notifyDataManagerChange(DataParameter<?> key) {
			if (DATA_HOOKED_ENTITY.equals(key)) {
				...
				this.caughtEntity = i > 0 ? this.world.getEntityByID(i) : null;
				System.out.println("Notify Data Manager. thread check:  "+this.world.isRemote);
			}
			if (POSITION_SYNC.equals(key)) {
				int State = (Integer) this.getDataManager().get(POSITION_SYNC);
				if (State == 0) {System.out.println("State is Flying!");}
				if (State == 1) {System.out.println("State is Hooked!");}
				if (State == 2) {System.out.println("State is Bobbing!");}
				//this.currentState = State.FLYING;
			}
		...
		public void onUpdate() {
			...
			//System.out.println("Caught Entity is "+caughtEntity);
			...
			if (this.angler == null) {
				...
				if (this.currentState == State.FLYING) {
					if (this.caughtEntity != null) {
						...
						this.getDataManager().set(POSITION_SYNC, 1);
						//TODO: States will not swap on client side, This is obviously a problem.
						return;
					}
					...
					if (f > 0.0F) {
						...
						this.getDataManager().set(POSITION_SYNC, 2);
						return;
					}
					...
					if (this.currentState == State.HOOKED_IN_ENTITY) {
						//System.out.println("State is HOOKED ENTITY!");
						//TODO
						//System.out.println("Hooked State Firing");
						if (this.caughtEntity != null) {
							//System.out.println("Hooked Entity is not null");
							if (this.caughtEntity.isDead) {
							//	System.out.println("Hooked Entity IS dead");
								this.caughtEntity = null;
								this.currentState = State.FLYING;
								this.getDataManager().set(POSITION_SYNC, 0);
							} else {
							//	System.out.println("Hooked Entity is not dead");
								System.out.println("Hooked Entity is "+caughtEntity);
								this.posX = caughtEntity.posX;
								
								d1 = (double) caughtEntity.height;
								
								this.posY = caughtEntity.getEntityBoundingBox().minY + d1 * 0.8D;
								
								this.posZ = caughtEntity.posZ;
								
								this.setPosition(this.posX, this.posY, this.posZ);
								
								System.out.println("Hook position is "+this.getPosition());
								System.out.println("Entity Position is "+caughtEntity.getPosition());
					...
					if (this.currentState == State.BOBBING) {
						//System.out.println("State is Bobbing!");
						this.motionX *= 0.9D;
						this.motionZ *= 0.9D;
		...
		private void checkCollision() {
			...
			double d1;
			while (true) {
				do {RayTraceResult raytraceresult1;do {do {do {
					if (!var8.hasNext()) {
		...
		private void setHookedEntity() {
			...
			this.getDataManager().set(POSITION_SYNC, 1);
			System.out.println("Set hooked entity to "+this.caughtEntity.getEntityId());
			System.out.println("Is the set fired on client"+this.world.isRemote);
			//TODO Render on Hooked entity properly 
		}
		...
		public void writeSpawnData(ByteBuf buffer) {
			...
			System.out.println("Spawn Data Write!");
		}
		
		
		public void readSpawnData(ByteBuf buffer) {
			Entity player = this.world.getEntityByID(buffer.readInt());
			if (player instanceof EntityPlayer) {
			this.init((EntityPlayer) player);
